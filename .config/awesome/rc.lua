...
tags = {
        names = {"main","hobby","work","irc","music","news",7,8,9},
...
-- doesn't work? start with .xinitrc instead
-- }}}
...
globalkeys = awful.util.table.join(
    ...
    -- Change volume --
    awful.key({ }, "XF86AudioRaiseVolume",  function () awful.util.spawn("pamixer -i 9") end),
    awful.key({ }, "XF86AudioLowerVolume",  function () awful.util.spawn("pamixer -d 9") end),
...
-- dec->hex
local function dec_hex(input)
    local base,span,out,i,d=16,"0123456789abcdef","",0
    while input>0 do
        i=i+1
        input,d=math.floor(input/base),(input%base)+1
        out=string.sub(span,d,d)..out
    end
    return out
end
...
local function bat_notification()
    ...
    local bat_status = f_status:read("*a")
    if(bat_status == "Discharging\n") then
        local green_hex,red_hex = dec_hex(bat_capacity),dec_hex(100-bat_capacity) 
        naughty.notify({text=bat_capacity, fg="#"..red_hex..green_hex.."00",bg="#000000"})
    end
    if(bat_capacity <= 10 and bat_status == "Discharging\n") then
        naughty.notify({ 
            title = "The battery is low",
            text = "The battery is low. " .. bat_capacity .. "%" .. ".",
            fg="#aa0a0a",
            bg="#000000",
            timeout = 15,
	    ...
	    awful.util.spawn("xautolock -locknow")
        awful.util.spawn("systemctl suspend")
    elseif(bat_capacity<=20 and bat_status == "Discharging\n") then
      naughty.notify({
            title = "The battery is NOT charging",
            text = "The battery is at " .. bat_capacity .. "%",
            fg="#aa0a0a",
            bg="#000000",
            timeout = 15,
            position = "bottom_left"
...
batterytimer:connect_signal("timeout", bat_notification)
batterytimer:start()
